...
class NotifyDailySuggestedProducts implements ShouldQueue
{
    ...
    {
        Log::info('Matchmaking buyers -> supplier');
        $results = collect([]);
        $attempts = 0;
            ...
            foreach($people as $person)
            {
               if($person->generalizedType() === 'supplier')
               {
                   try
                   {
                       ...
                       {
                           $matches = $this->matchingService->getMatchesForSupplier($person);
                           if(count($matches) > 0)
                           {
                               $results->push([
                                   'person' => $person,
        ...
        {
            Log::info('Matchmaking unsuccessful for suppliers');
        }
    }
}
